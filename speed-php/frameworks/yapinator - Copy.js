	...
	Yapinator = (function() {
		...
		core = {
			...
			getAttr: function( el, val ) {
				...
				if( val === "for" )
					return el.htmlFor;	
				return el.getAttribute( val ) || el[ val ];
			},
			// make array
			makeArray: function( arr ) {
				if( !!( window.attachEvent && !support.opera ) ) {
			...
			byClass: function( cls, root, tag ) {
				if ( support.bycls && !tag )
					return core.makeArray( root.getElementsByClassName( cls ) );
				else
					return this.byClassTag( cls, root, tag );
			},
			// attributes processing function [attr=val]
			fnAttr: function( sel, root, tag ) {
				var tag = tag || "*", nodes = [], els, j = 0, l, el, m, attr, s, val;
				els = root.getElementsByTagName( tag );
				selectors.reg.attr.lastIndex = 0;
				while ( ( m = selectors.reg.attr.exec( sel ) ) !== null ) {
					attr = m[1], s = m[2] || "", val = m[3] || "";
					nodes = []; j = 0; l = els.length;
					while( l-- ) {
						el = els[ j ];
						// check either attr is defined for given node or it's equal to given value
						if ( selectors.attr[ s ] && ( selectors.attr[ s ] ( el, attr, val ) ) ) {
							nodes.push( el );
						}
						j++;
					}
			...
			fnPseudo: function( sel, root, tag, n ) {
				var tag = tag || "*", nodes = [], els, el, j = 0, l, cnt;
				els = root.getElementsByTagName( tag );
				cnt = l = els.length;
				while( cnt-- ) {
					el = els[ j ];
					if ( selectors.pseudo[ sel ] && !selectors.pseudo[ sel ]( el, n, j, l ) ) {
						nodes.push( el );
					}
					j++;
				}
			...
			getPseuNth: function( root, typ, nth, nthrun ) {
				switch( typ ) {
					case "not":
						return Yapinator.select( nth, root, null, true, true, nthrun );
					default:
...
//					return ( " " + core.getAttr( el, attr ) + " " ).indexOf( " " + val + " " ) != -1;
				...
				"$=": function ( el, attr, val ) {
					return ( attr = core.getAttr( el, attr ) + "") && attr.indexOf( val ) == attr.length - val.length;
				},
			...
			pseudo: {
				...
				nthChild: function ( el, n, t ) {
					...
					if ( !el.nIdx ) {
						var node = el.parentNode.firstChild, cnt = 0;
						for( ; node; node = node.nextSibling ) {
							if ( !t ? node.nodeType == 1 : node.nodeType == 1 && node.nodeName == el.nodeName )
								node.nIdx = ++cnt;
						}
					}
...
//					return el.parentNode.getElementsByTagName('*').length != 1;
				},
...
//					return el.parentNode.getElementsByTagName('*').length != 1;
				...
				// E:parent - an E element that has no children (including text nodes)
				parent: function( el ) {
		...
		return {
			...
			version: version,
			qsa: support.qsa,
			makeArray: core.makeArray,
			mergeArray:core.mergeArray,
			// main selector function
			select: function( selector, root, results, noCache, loop, nthrun ) {
				...
				if ( ( m = selectors.reg.sharpTest.exec( selector ) ) !== null ) {
					switch( m[2] ) {
						// Speed-up: ("#id")
						case "#":
							set = core.byId( m[3], root );
							break;
						// Speed-up: (".class")
						case ".":
							var cls = m[3].replace( /\./g, " " );
							set = core.byClass( cls, root, m[1] );
							break;
						default:
							// Speed-up: ("body")
							...
							// Speed-up: ("tag")
								...
								set = core.makeArray( root.getElementsByTagName ( m[0] ) );
					}
				} else {
					...
					if ( support.qsa ) {
						try {
							...
							cache[ selector ] = set;
							return (rets && !loop) ? core.mergeArray( set, rets ) : set;
						} catch ( qsaExp ) {}
					}
					// attribute
					if ( selectors.reg.attr.test( selector ) ) {
						var tag = selectors.reg.aTag.exec( selector )[1];
	...
	if ( Yapinator.qsa ) {
		(function(){
			var oldYap = Yapinator.select;
			
			Yapinator.select = function( selector, root, results, noCache, loop, nthrun ) {
				// Return cache if exists
				// Return no cached result if root specified
				if ( cache[ selector ] && !noCache && !root ) {
					return cache[ selector ];
				...
				try {
					set = root.querySelectorAll( selector );
					cache[ selector ] = set;
					return set(rets && !loop) ? Yapinator.mergeArray( set, rets ) : set;
				} catch( qsaExp ){}
				
				return oldYap.apply(oldYap, Yapinator.makeArray( arguments ) );
			};
		})();
	} else {
		// Loader
	...
	win.Yapinator = Yapinator.select;
